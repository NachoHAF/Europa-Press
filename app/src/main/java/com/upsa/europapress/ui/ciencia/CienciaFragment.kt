package com.upsa.europapress.ui.ciencia
...
import com.upsa.europapress.databinding.FragmentCienciaBinding
import www.sanju.motiontoast.MotionToast

class CienciaFragment : Fragment() {

    private lateinit var viewModel: CienciaViewModel
    private lateinit var parser: Parser
    private lateinit var adapterArticle: ArticleAdapter

    private var _binding: FragmentCienciaBinding? = null
    private val binding
        get() = _binding!!

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        _binding = FragmentCienciaBinding.inflate(inflater, container, false)
        // 初始化
        adapterArticle = ArticleAdapter(mutableListOf(), Channel())
        binding.recyclerViewCiencia.adapter = adapterArticle
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 初始化
        viewModel = ViewModelProvider(this)[CienciaViewModel::class.java]
        parser = Parser.Builder().context(requireContext()).cacheExpirationMillis(10L * 60L * 100L).build()

        // 新闻列表
        val recyclerView = binding.recyclerViewCiencia
